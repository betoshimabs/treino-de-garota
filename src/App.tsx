...
export function WorkoutPage({ data, refresh, setNotice, allExercises, allTemplates }: SharedProps) {
  ...
  const filtered = allExercises.filter((exercise) => {
    const inSection = pickerCategory === 'all' || (pickerCategory === 'favorites' ? data.favorites.includes(exercise.id) : pickerCategory === 'strength' ? exercise.category === 'strength' : exercise.category !== 'strength')
...
{creatingManual && <ManualExerciseSheet exercises={allExercises} onUseExisting={async exercise => { await addExercise(exercise); setCreatingManual(false); }} addToWorkout onClose={() => setCreatingManual(false)} onSaved={async exercise => { await addExercise(exercise); await refresh(); setCreatingManual(false); setNotice('Exercício manual criado e adicionado.'); }} />}
...
export function ExercisesPage({ data, refresh, setNotice, allExercises, allTemplates }: SharedProps) {
  const [query, setQuery] = useState('')
  const [group, setGroup] = useState('Todos')
  const [creating, setCreating] = useState(false)
  const [creatingTemplate, setCreatingTemplate] = useState(false)
  const [librarySection, setLibrarySection] = useState<'exercises' | 'templates'>('exercises')
  const filtered = allExercises.filter((exercise) => (group === 'Todos' || (group === 'Favoritos' ? data.favorites.includes(exercise.id) : exercise.group === group)) && `${exercise.name} ${exercise.aliases.join(' ')}`.toLocaleLowerCase().includes(query.toLocaleLowerCase()))
  ...
  return (
    <div className="page exercises-page">
      ...
      {creating && <ManualExerciseSheet exercises={allExercises} onUseExisting={async exercise => { setCreating(false); window.location.hash = `/exercicios/${exercise.id}`; }} onClose={() => setCreating(false)} onSaved={async () => { await refresh(); setCreating(false); setNotice('Exercício manual criado. O envio para revisão acontece quando houver conexão.') }} />}
